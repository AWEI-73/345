...
class PTTScraper:
    ...
    def get_ptt_stock_posts(self, url: str = "https://www.ptt.cc/bbs/Stock/index.html", num_posts: int = 5) -> List[Dict]:
        try:
...
# --- Streamlit UI ---
st.set_page_config(page_title="小股神助理 (Gemini API 版)", layout="wide")
st.title("小股神助理 (Gemini API 雲端部署版)")
...
if st.button("更新 PTT 文章"):
    with st.spinner("正在獲取 PTT 股票版文章..."):
        try:
            ...
            traceback.print_exc()
# 運行 Streamlit App: 在終端機中輸入 `streamlit run stockk_pui.py` 
